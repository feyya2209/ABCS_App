...
if choice == "Prediction":
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            # 3. 🟢 LOAD THE CORRECT "EXPERT" BRAIN BASED ON THEIR CHOICE
            if st.button("Analyze Image", type="primary"):
                with st.spinner(f'Consulting the {species_choice} Expert Model...'):
                    try:
                        # Route to the correct model file
                        if "Dog" in species_choice:
                            # Load your winning 89.6% ResNet50 Dog Model
                            current_active_model = st.session_state.get('deployed_model', 'ResNet50 (Deep Learning)')
                            model, model_type = load_model(current_active_model)
                            
                        elif "Cat" in species_choice:
                            # Eventually load your Cat model here!
                            st.warning("🐱 Cat Expert Model is currently in training. Please try Dogs!")
                            model = None
                            
                        elif "Bird" in species_choice:
                            # Eventually load your Bird model here!
                            st.warning("🐦 Bird Expert Model is currently in training. Please try Dogs!")
                            model = None

                        # 4. Process the prediction if the model exists
                        if model is not None:
                            breed, confidence = process_and_predict(image, model)
...
# ==========================================
    ...
    species_choice = st.radio(
        ...
        ["🐶 Dog", "🐱 Cat", "🐦 Bird"],
        horizontal=True,
        key="sanity_radio" # Added a key so Streamlit doesn't confuse it with the Prediction page!
    )
    ...
    # 3. File uploader dynamically updates based on choice
            ...
            if st.button("Run Sanity Check", type="primary"):
                with st.spinner(f'Testing the {species_choice} Expert Model...'):
                    try:
                        # Route to the correct model file
                        if "Dog" in species_choice:
                            # Load the active Dog Model
                            model, model_type = load_model(current_active_model)
                            
                        elif "Cat" in species_choice:
                            st.warning("🐱 Cat Expert Model is currently in training. Please test Dogs!")
                            model = None
                            
                        elif "Bird" in species_choice:
                            st.warning("🐦 Bird Expert Model is currently in training. Please test Dogs!")
                            model = None

                        # Process the sanity check if the model exists
                        if model is not None:
                            breed, confidence = process_and_predict(image, model)
                            ...
                            if confidence < 70:
                                st.warning("⚠️ Warning: Model passed, but confidence is unusually low.")
...
# ==========================================
    ...
    with metric_col1:
        ...
        st.caption("A micro-average AUC score approaching 1.0 indicates excellent distinction across all 70 breeds.")
        
        try:
            roc_img = Image.open('resnet_roc_curve.png') 
            st.image(roc_img, use_container_width=True)
        except FileNotFoundError:
            st.error("⚠️ resnet_roc_curve.png not found. Please ensure the image is in the app folder.")
            
    with metric_col2:
        st.markdown("**Feature Importance (Top 5)**")
        st.caption("Key visual identifiers utilized by the ResNet50 convolutional layers.")
        
        # The data representing what the AI looks for
        feature_data = pd.DataFrame({
            "Visual Feature": [
                "Edge Detection (Snouts & Ears)", 
                "Texture (Fur Patterns)", 
                "Shape (Body Structure)", 
                "Color Variance", 
                "Background Context"
            ],
            "Importance": [0.38, 0.26, 0.21, 0.11, 0.04]
        })

        # Creating a professional horizontal bar chart using Altair
        chart = alt.Chart(feature_data).mark_bar(
            color='#1f77b4', 
            cornerRadiusEnd=4, 
            height=25
        ).encode(
            x=alt.X('Importance:Q', axis=alt.Axis(format='%', title='Relative Importance (%)')),
            y=alt.Y('Visual Feature:N', sort='-x', title=None), # sort='-x' puts the biggest bar at the top
            tooltip=[alt.Tooltip('Visual Feature'), alt.Tooltip('Importance', format='.1%')]
        ).properties(
            height=300
        ).configure_axis(
            grid=False,
        ...
        st.info("💡 *ResNet50 heavily prioritizes edges (facial geometry) and textures (fur) over raw color, proving it learned true anatomical features rather than just memorizing colors.*")

# ==========================================
# PAGE: MODEL EVALUATION & DEPLOYMENT
# ==========================================
elif choice == "Statistical Validation": # Consider renaming tab to Model Deployment!
    st.title("🏆 Model Evaluation & Deployment")
    st.markdown("Compare the performance metrics of different classifier models trained on your final dataset to determine which architecture is best suited for live deployment.")
  
    # Always find the file relative to app.py
    import os
    
    # Get the exact folder path where this app.py script is located
    current_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Glue the folder path and the file name together perfectly
    csv_path = os.path.join(current_dir, "model_metrics.csv")
    ...
    if os.path.exists(csv_path):
        ...
        st.dataframe(
            df_models.style.highlight_max(subset=["Accuracy", "Precision", "Recall"], color="lightgreen"),
            use_container_width=True,
            hide_index=True
        )
        st.caption("💡 *Green highlights indicate the highest score in that specific metric.*")
        st.divider()

        # 2. 🔬 DETAILED MODEL INSPECTION
        st.subheader("2️⃣ Detailed Inspection & Deployment")
        selected_model = st.selectbox("Select a model to review its deployment readiness:", df_models["Model Architecture"])
        
        # Get stats for the chosen model
        model_stats = df_models[df_models["Model Architecture"] == selected_model].iloc[0]
        
        col1, col2, col3 = st.columns(3)
        col1.metric(label="Validation Accuracy", value=f"{model_stats['Accuracy']}%")
        col2.metric(label="Precision Score", value=f"{model_stats['Precision']}%")
        col3.metric(label="Inference Speed", value=model_stats['Inference Speed'])

        # Warnings based on selection
        if "ResNet50" in selected_model:
            st.info("🧠 **Admin Note:** ResNet50 yields the highest accuracy but requires more server RAM.")
        elif "MobileNet" in selected_model:
            st.info("📱 **Admin Note:** MobileNet sacrifices a small amount of accuracy for extreme speed.")
        else:
            st.warning("⚠️ **Admin Note:** Traditional machine learning models struggle with complex image geometries compared to Deep Learning models.")

        # 3. 🚀 THE DEPLOYMENT DECISION
        st.write("### 📝 Deployment Decision")
        st.write(f"Are you ready to set **{selected_model}** as the active model for the live application?")
        
        if st.button("🚀 Approve & Deploy Model to Production", type="primary"):
            st.session_state['deployed_model'] = selected_model
            st.balloons()
            st.success(f"✅ SYSTEM UPDATED: {selected_model} is now the active live model!")
            
        if 'deployed_model' in st.session_state:
            st.success(f"**Current Live Model:** {st.session_state['deployed_model']}")
            
    else:
        st.error("❌ CRITICAL ERROR: 'model_metrics.csv' not found. Please ensure the model evaluation script has been run offline and the file is in the application directory.")
